...
extension LoginViewController: ASAuthorizationControllerDelegate, ASAuthorizationControllerPresentationContextProviding {
        ...
        if let appleIDCredential = authorization.credential as? ASAuthorizationAppleIDCredential {
            ...
            Auth.auth().signIn(with: credential) { (authResult, error) in
                if  let user = authResult?.user {
                    ...
                    self.userManager.fetchUsers { result in
                        switch result {
                        case .success(let users):
                            self.existingUsers = users
                            if self.mappingUserInfo(user.uid) == true {
                                ...
                                self.userManager.createUser(user: &, completion: <#T##(Result<String, Error>) -> Void#>)
    ...
    func mappingUserInfo(_ uid: String) -> Bool {
